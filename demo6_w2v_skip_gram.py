...
filename = './data/demo6_w2v/text8.zip'
...
def build_dataset(words):
    ...
    # 将words中的每个分词用序列号表示:[14880, 4491, 483, 70, 1, 1009, 1850, 317, 14, 76]

    count[0][1] = unk_count
    reverse_dictionary = dict(zip(dictionary.values(),dictionary.keys()))
    # 将dictionary中的key和value对换:[(0, 'UNK'), (1, '的'), (2, '在'), (3, '是'), (4, '年'), (5, '和'), (6, '为'), (7, '了'), (8, '有'), (9, '与')]
    # 相当于key是编号，value是对应的词
    return data, count, dictionary, reverse_dictionary

data, count, dictionary, reverse_dictionary = build_dataset(words)
# data：2262896,语料中的每个词的对应的编号； count:199247，相当于词频表，key是语料中所有的词，value是词频；
...
def generate_batch(batch_size, num_skips, skip_window):
    ...
    assert num_skips <= 2 * skip_window  #num_skips 目标词左右的上下文多少个   skip_window是上文或下文多少个
    batch = np.ndarray(shape=(batch_size), dtype=np.int32)    #  1 dim   array[1,2,3,4,5,6]
    labels = np.ndarray(shape=(batch_size, 1), dtype=np.int32)   # labels 8行，1列，
...
for j in range(10):
    batch, labels = generate_batch(batch_size=8, num_skips=4, skip_window=2)  # num_skip=2 就是取上文
    # skip_window=2代表着选取左input word左侧2个词和右侧2个词进入我们的窗口，所以整个窗口大小span=2x2+1=5
    # num_skips，它代表着我们从整个窗口中选取多少个不同的词作为我们的output word
    for i in range(8):
        print(batch[i], reverse_dictionary[batch[i]],
              '->', labels[i, 0], reverse_dictionary[labels[i, 0]])

# while True: input('>>>>') # sys.stdin.read()
...
with graph.as_default():
    ...
    # embedding_lookup(params, ids) 按照ids顺序返回params中的第ids行
...
with tf.Session(graph=graph, config=config) as session:
    ...
    fp = open('./data/demo6_w2v/vector_skip_gram.txt', 'w', encoding='utf8')
...
def plot_with_labels(low_dim_embs, plot_labels, filename='./data/demo6_w2v/tsne_skip_gram.png'):
