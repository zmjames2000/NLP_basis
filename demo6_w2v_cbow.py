...
filename = './data/demo6_w2v/test8.zip'
...
vocabulary_size = len(set(words)) # words 中不重复的分词数量  #199247
...
with graph.as_default():
    ...
    embeddings = tf.Variable(
        tf.random_uniform([vocabulary_size, embedding_size], -1.0, 1.0)) # xxx 128
...
with tf.Session(graph=graph) as session:
    ...
    for step in range(num_steps):
        ...
        fp = open('./data/demo6_w2v/vector_cbow.txt', 'w', encoding='utf8')
...
def plot_with_labels(low_dim_embs, plot_labels, filename='./data/demo6_w2v/tsne_cbow.png'):
